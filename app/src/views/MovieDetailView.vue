...
import { formatDuration, formatPrice, formatShowtime, lowestTierPrice } from '@/lib/format'
...
<template>
  <div class="min-h-screen bg-base">
    ...
    <main class="mx-auto max-w-6xl px-4 py-8 md:px-6">
      ...
      <div v-if="loading" class="py-16 text-center text-copy-secondary">Loading movie…</div>
      ...
      <div v-else-if="detail" class="grid gap-8 lg:grid-cols-[280px_1fr]">
        ...
        <div class="flex flex-col gap-6">
          ...
          <section>
            <h2 class="mb-4 text-lg font-medium text-copy-primary">Showtimes</h2>

            <div
              v-if="detail.showtimes.length === 0"
              class="rounded-lg border border-surface-border bg-subtle px-4 py-8 text-center"
            >
              <p class="text-copy-primary">Showtimes not yet announced</p>
              <p class="mt-1 text-sm text-copy-secondary">
                Check back later for screening times at this cinema.
              </p>
            </div>

            <div v-else class="grid gap-3 sm:grid-cols-2">
              <Card
                v-for="showtime in detail.showtimes"
                :key="showtime.id"
                ...
                <CardContent class="flex items-center justify-between gap-4 p-4">
                  <div>
                    <p class="font-medium text-copy-primary">
                      {{
                        formatShowtime(showtime.startsAt, selectedCinemaMeta?.timezone)
                      }}
                    </p>
                    <p class="text-sm text-copy-secondary">{{ showtime.screenName }}</p>
                    <p class="mt-1 text-xs text-copy-muted">
                      from
                      {{ formatPrice(lowestTierPrice(showtime.priceTiers)) }}
                    </p>
                  </div>
