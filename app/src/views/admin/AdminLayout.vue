...
const navItems = [
  ...
  { to: '/admin/bookings', label: 'Bookings', disabled: true },
  { to: '/admin/scan', label: 'Scan', disabled: true },
  { to: '/admin/logs', label: 'Logs', disabled: true },
]
</script>
    ...
    <aside class="flex w-56 shrink-0 flex-col border-r border-surface-border bg-surface">
      ...
      <nav class="flex-1 space-y-1 p-3">
        <RouterLink
          ...
          :class="
            item.disabled
              ? 'cursor-not-allowed text-copy-faint'
              : 'text-copy-secondary hover:bg-accent-dim hover:text-brand'
          "
        ...
        >
          {{ item.label }}
          <span v-if="item.disabled" class="ml-1 text-xs text-copy-faint">(soon)</span>
        </RouterLink>
      </nav>
